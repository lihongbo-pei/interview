...
在我们原来的检测系统中，图片和视频文件是直接保存在本地文件夹的，这种方式在早期还可以满足需求，但随着检测任务量增大、文件数量和体积迅速增长，存在几个明显问题：

1. **可扩展性差**：本地存储容量有限，难以支持后期的横向扩展；不同实例之间的文件无法共享，影响了分布式部署。
2. **可靠性低**：一旦服务器故障或迁移，文件容易丢失或路径失效，不利于备份与恢复。
3. **访问效率不足**：视频文件较大，本地访问对IO造成压力，上传和下载速度不稳定。

因此我将文件存储升级为 **MinIO 对象存储**。MinIO 兼容 S3 协议，可以与主流云服务对接，也能本地自建，具备高可靠性、分布式扩展、版本控制等特性。

通过接入 MinIO，我实现了：

- **文件与计算分离**：检测服务只关注算法推理，不受文件系统限制；
- **统一访问接口**：所有检测结果文件都通过 URL 直接访问，方便前端展示与外部系统对接；
- **更安全与可控**：通过 AccessKey/SecretKey 控制访问权限，支持多租户与私有部署；
- **易扩展**：后续可以直接对接云端对象存储（如阿里OSS、AWS S3），提升系统的云原生能力。

总体上，这次升级让系统更**高可用、可扩展、云原生化**，也为后续的**多实例部署与分布式检测**打下了基础。
